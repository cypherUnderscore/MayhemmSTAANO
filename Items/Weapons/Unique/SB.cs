...
{
	public class SB : ModItem
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Scaleblaster");
		...
		public override bool Shoot(Player player, ref Vector2 position, ref float speedX, ref float speedY, ref int type, ref int damage, ref float knockBack)
		{
			...
			if (bulletDecider == 0) //Supershot
			{
				...
				for (int s = 0; s < 3; s++)
				{
					Vector2 randomSpread = new Vector2(speedX, speedY).RotatedByRandom(MathHelper.ToRadians(8));
					Projectile.NewProjectile(position, randomSpread, mod.ProjectileType("SBProj"), damage, knockBack, player.whoAmI);
				}
				return false;
			}
